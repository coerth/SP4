...
public class Player extends Entity implements PlayerI{
    ...
    public void movement() {
        if(super.getpApplet().keyCode == VK_W){
            pVector = new PVector(pVector.x, pVector.y-1*scale);
        }else if(super.getpApplet().keyCode == VK_S){
            pVector = new PVector(pVector.x, pVector.y+1*scale);
        }else if(super.getpApplet().keyCode == VK_A){
            pVector = new PVector(pVector.x - 1*scale, pVector.y);
        ...
        if (pVector.x < 0 ) {
            ...
            pVector.y=0;
        }else if( pVector.x > getpApplet().width  - scale){
            pVector.x= getpApplet().width  - scale;
        }else if( pVector.y > getpApplet().height  - scale){
            pVector.y = getpApplet().height - scale;
        }

        System.out.println("X er: "+pVector.x);
        System.out.println("Y er: "+pVector.y);
